...
export const fullAssessmentScenario = {
  ...
  onboarding: {
    ...
    // The clinical overlay intentionally keeps only the facts needed to shape
    // the questionnaire. The browser action must answer every required
    // treatment-history section before it can continue, so keep that complete
    // raw wire fixture separate from the clinical fixture.
    intakeWireStepData: {
      "treatment-history": {
