import { NextRequest } from "next/server";
import { afterEach, beforeEach, describe, expect, it, vi } from "vitest";

const TEST_TOKEN = "test-support-token-with-at-least-32-chars";
const RUN_ID = "123e4567-e89b-42d3-a456-426614174000";
const EMAIL = `casey.assessment.${RUN_ID}@e2e.example.com`;
const BODY = { run_id: RUN_ID, email: EMAIL, fixture: "results-report-v1" };

function makeRequest(body: unknown, token = TEST_TOKEN): NextRequest {
  return new NextRequest(
    "https://patient-web.example.com/api/test/results-fixture",
    {
      method: "POST",
      headers: {
        authorization: `Bearer ${token}`,
        "content-type": "application/json",
      },
      body: JSON.stringify(body),
    },
  );
}

describe("patient web results-fixture support route", () => {
  beforeEach(() => {
    vi.stubEnv("ENVIRONMENT", "test");
    vi.stubEnv("PATIENT_WEB_TEST_SUPPORT_ENABLED", "true");
    vi.stubEnv("PATIENT_WEB_TEST_SUPPORT_TOKEN", TEST_TOKEN);
    vi.stubEnv("BACKEND_INTERNAL_URL", "http://backend.internal");
    vi.stubEnv(
      "PATIENT_WEB_CSRF_SECRET",
      "results-fixture-test-csrf-secret-at-least-32-bytes",
    );
    vi.stubEnv("PATIENT_WEB_CLIENT_IP_MODE", "single-bucket");
    vi.stubEnv("PATIENT_WEB_CREDENTIAL_RATE_LIMIT_STORE", "memory");
    vi.stubEnv("PATIENT_WEB_ALLOWED_ORIGINS", "https://patient.example.test");
  });

  afterEach(() => {
    vi.resetModules();
    vi.unstubAllEnvs();
    vi.unstubAllGlobals();
  });

  it("forwards only the named fixture and returns a fixed acknowledgement", async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      Response.json({
        status: "fixture_ready",
        fixture: "results-report-v1",
        report: { secret: "must-not-leak" },
      }),
    );
    vi.stubGlobal("fetch", fetchMock);
    const { POST } = await import("./route");

    const response = await POST(makeRequest(BODY));

    expect(response.status).toBe(200);
    expect(response.headers.get("cache-control")).toContain("no-store");
    expect(await response.json()).toEqual({
      status: "fixture_ready",
      fixture: "results-report-v1",
    });
    expect(fetchMock).toHaveBeenCalledWith(
      new URL("/test/results-fixture", "http://backend.internal"),
      expect.objectContaining({ body: JSON.stringify(BODY) }),
    );
  });

  it.each([
    { ...BODY, unexpected: true },
    { ...BODY, run_id: "223e4567-e89b-42d3-a456-426614174000" },
    { ...BODY, fixture: "other-fixture" },
  ])("rejects non-exact fixture requests: %j", async (body) => {
    const fetchMock = vi.fn();
    vi.stubGlobal("fetch", fetchMock);
    const { POST } = await import("./route");

    expect((await POST(makeRequest(body))).status).toBe(404);
    expect(fetchMock).not.toHaveBeenCalled();
  });
});
