...
const { POST: setup } =
  await import("@/app/api/auth/mfa/enrollment/setup/route");
const { POST: confirm } =
  await import("@/app/api/auth/mfa/enrollment/confirm/route");
...
const { POST: authenticatedSetup } =
  await import("@/app/api/auth/mfa/setup/route");
const { POST: authenticatedConfirm } =
  await import("@/app/api/auth/mfa/confirm/route");
...
describe("exact MFA BFF routes", () => {
  ...
  it("injects the HttpOnly transaction into login verification", async () => {
    ...
    expect(response.headers.getSetCookie().join("\n")).toContain(
      "spine_patient_sess=access",
    );
  });

  it("audits missing MFA transaction denial without token or code values", async () => {
    const response = await verify(
      request("/api/auth/mfa/verify", "POST", { code: "private-code" }),
    );
    ...
    expect(mockedAuditLog).toHaveBeenCalledWith(
      expect.objectContaining({
        ...
        method: "POST",
        status: 401,
        reason: "mfa_transaction_missing",
      }),
    );
    const auditOutput = JSON.stringify(mockedAuditLog.mock.calls);
    expect(auditOutput).not.toContain("private-code");
    expect(auditOutput).not.toContain("spine_patient_mfa_transaction");
  });
