...
export async function POST(request: NextRequest) {
  ...
  auditLog({ ts: new Date().toISOString(), event: 'auth.mfa.verify.attempt', method: 'POST', requestId })
  ...
  try {
    response = await forwardCredentialAuth('/api/v1/auth/mfa/verify', body, request)
  } catch (err) {
