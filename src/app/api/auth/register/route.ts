...
function registrationResponseBody(data: JsonRecord): JsonRecord {
  const { verification_token: verificationToken, ...safeBody } = data;
  if (
    typeof verificationToken === "string" &&
    verificationToken.trim().length > 0
  ) {
    return { ...safeBody, registration_verification_pending: true };
  }
  return safeBody;
}
...
export async function POST(request: NextRequest) {
  ...
  const rateLimitResult = await checkCredentialRateLimit(
    request,
    "auth.register",
    {
  ...
  if (
    rateLimitResult === "client_ip_unavailable" ||
    rateLimitResult === "store_unavailable"
  ) {
    auditLog({
      ts: new Date().toISOString(),
      event: "auth.register.failure",
  ...
  if (body == null) {
    ...
    return clearAccountTransitionState(
      jsonNoStore({ error: "invalid_json" }, { status: 400 }),
    );
  }
  ...
  try {
    response = await forwardCredentialAuth(
      ...
      undefined,
      {
        ...
        responseBody: registrationResponseBody,
        onSafeResponse: (safeResponse, backendData) => {
          const verificationToken = backendData.verification_token;
          if (
            typeof verificationToken === "string" &&
            verificationToken.trim().length > 0
          ) {
            setRegistrationVerificationCookie(safeResponse, verificationToken);
          }
        },
      },
    );
