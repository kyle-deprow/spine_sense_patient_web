...
export const COOKIE_NAMES = {
  ...
  registrationVerification: "spine_patient_registration_verify",
} as const;
...
export const REGISTRATION_VERIFICATION_MAX_AGE_SECONDS = 10 * 60;
...
export function shouldUseSecureCookies(
  ...
  if (override === "false") {
    return !(
      nodeEnv === "development" ||
      (allowInsecureE2e === "true" && hasOnlyLocalOrigins(allowedOrigins))
    );
  }
...
export function registrationVerificationCookieOptions(): CookieOptions {
  return {
    httpOnly: true,
    secure: shouldUseSecureCookies(),
    sameSite: "strict",
...
export function setMfaTransactionCookies(
...
export function setMfaPendingCookie(
  ...
  if (!pendingId)
    throw new Error("Cannot issue an empty MFA pending identifier");
  response.cookies.set(
...
export function setRegistrationVerificationCookie(
  response: NextResponse,
  verificationToken: string,
): void {
  if (!verificationToken.trim())
    throw new Error("Cannot issue an empty registration verification token");
  response.cookies.set(
    COOKIE_NAMES.registrationVerification,
    verificationToken,
    registrationVerificationCookieOptions(),
  );
}

export function clearRegistrationVerificationCookie(
  response: NextResponse,
): void {
  clearCookie(
    response,
...
export function tokenSessionCorrelation(
  accessToken: string,
...
export function verifyAuditActorCookie(
  ...
  if (!key || correlation !== tokenSessionCorrelation(accessToken, key))
    return undefined;

  const payload = `${version}.${keyId}.${actorId}.${issuedAt}.${correlation}`;
  const expected = Buffer.from(
    auditActorSignature(payload, key.secret),
    "base64url",
  );
...
export function auditActorIdFromRequest(
  request: NextRequest,
): string | undefined {
...
export function issueAuthenticatedSessionCookies(
  ...
  if (
    ...
    throw new Error(
      "Cannot issue patient web session without a full token pair and trusted actor",
    );
  }
  ...
  return {
    actorId,
    issuedAt,
    sessionCorrelation: tokenSessionCorrelation(
      session.accessToken,
      auditActorSigningKeys.current,
    ),
  };
...
function validSessionIssuedAt(
  ...
  return (
    ...
    value > 0 &&
    value <= now + 60 &&
    now - value <= SESSION_MAX_AGE_SECONDS
  );
}
