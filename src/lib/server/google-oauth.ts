...
export async function completeGoogleOAuth(
  ...
  const mode = parseMode(
    request.cookies.get(OAUTH_COOKIE_NAMES.mode)?.value ?? null,
  );
  const returnTo = safeReturnTo(
    request.cookies.get(OAUTH_COOKIE_NAMES.returnTo)?.value ?? null,
  );
  ...
  try {
    ...
    const data = await readJsonBody<
      BackendLoginResponse & BackendErrorResponse
    >(backendResponse);
    ...
    if (!backendResponse.ok) {
      return googleFailureRedirect(
        request,
        mode,
        googleFailureReason(mode, backendResponse.status, data),
      );
    }
