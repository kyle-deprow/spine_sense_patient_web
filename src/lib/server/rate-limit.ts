  ...
  return {
    async consume(key, limit, windowMs) {
      ...
      if (result !== 0 && result !== 1)
        throw new Error("Unexpected credential rate-limit result");
      return result === 1;
    },
    async clear() {
      const client = await getReadyRedisClient(redisUrl);
...
function configuredStore(): RateLimitStore {
  ...
  if (config.redisUrl === null)
    throw new Error("Redis credential rate-limit store is unavailable");
...
function validateOptions(opts: { limit: number; windowMs: number }): void {
  ...
  if (!Number.isSafeInteger(opts.windowMs) || opts.windowMs < 1) {
    throw new Error(
      "Credential rate-limit windowMs must be a positive integer",
    );
  }
}
...
export function shouldBypassRateLimit(
  ...
  if (
    !["local", "development", "dev", "test", "e2e"].includes(
      environment?.trim() ?? "",
    ) ||
