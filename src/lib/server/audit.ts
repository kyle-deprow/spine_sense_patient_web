...
const ROUTINE_AUDIT_ENVIRONMENTS = new Set([
  "local",
  "development",
  "dev",
  "test",
...
export function createAuditContext(token?: string): AuditContext {
  ...
  const sessionCorrelation = token
    ? sessionCorrelationFromToken(token)
    : undefined;
  return sessionCorrelation === undefined
    ? { requestId }
    : { requestId, sessionCorrelation };
}
...
export function backendAuthenticatedActorId(
  value: unknown,
): string | undefined {
  return normalizeAuditActorId(value);
}

export function isRoutineAuditEnabled(): boolean {
  if (process.env.NODE_ENV === "development") return true;
  const environment = process.env.ENVIRONMENT?.trim().toLowerCase();
  return (
    environment !== undefined && ROUTINE_AUDIT_ENVIRONMENTS.has(environment)
  );
}
...
function safeLabel(value: string | undefined): string | undefined {
  if (value === undefined || value.length > 80 || !SAFE_LABEL_RE.test(value))
    return undefined;
  return value;
}
